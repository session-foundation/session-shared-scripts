"""A ticket's conversation in English, for the agent who cannot read the original.

Written to a custom field by the digest before it posts, so every ticket that can
reach the reply dialog already carries one, and on demand by the `claude: english`
note. Both go through translate(): the model translates, and nothing else.
"""
import json
import os
import sys
...
def translate(model, turns, ticket_id):
    """The turns in English: (turns, None), or (None, why) when the model call failed.

    Python owns the speaker labels and timestamps and the model only translates. Asked
    to format the transcript itself, a model can drop a turn, merge two, or date one it
    was never given, and every one of those is invisible in the output.
    """
    payload = json.dumps([{"index": t["index"], "speaker": t["who"], "text": t["body"]}
                          for t in turns], ensure_ascii=False)
    rendered, failure = claude_cli.try_run_json(
        model, "medium", TRANSCRIPT_SYSTEM_PROMPT, TRANSCRIPT_SCHEMA,
        clip(payload, TRANSCRIPT_INPUT_CHARS), ENGLISH_TIMEOUT_SECONDS,
        f"the English transcript of #{ticket_id}")
    if failure:
        return None, failure
    return rendered.get("turns"), None
...
def write_english_field(session, subdomain, ticket_id, field_id, english):
    ...
    order.
    """
    fields = {"custom_fields": [{"id": field_id, "value": english}]}
    try:
        resp = zendesk.update_ticket(session, subdomain, ticket_id, fields, attempts=2)
...
def attach_english(session, subdomain, tickets, findings, model, field_id):
    ...
    for finding in candidates:
        ...
        if not turns:
            ...
            continue
        rendered, failure = translate(model, turns, ticket["id"])
        if failure:
            print(f"Note: could not render #{ticket['id']} in English ({failure}).")
            continue
        english = clip(render_transcript(turns, rendered), TRANSCRIPT_CHARS)
