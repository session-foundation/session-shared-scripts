"""The Zendesk account as the scripts here see it.

The API, through one authenticated session: search, one ticket, its comments and
users, the one PUT every write is. And the shapes in it that more than one script
reads: the markers the tools leave on comments, who the customer is on a channel
integration, what an imported app-store review looks like.

Every read names its own failure policy, because the callers differ: a digest
enriching many tickets skips the one it cannot read, while a command acting on one
ticket has nothing to do without it and exits.
"""
import os
import re
import sys
...
def fetch_tickets(session, subdomain, query, max_tickets):
    ...
    """
    base = api_url(subdomain, "search.json")
    url = base
...
def fetch_ticket(session, subdomain, ticket_id):
    url = api_url(subdomain, f"tickets/{ticket_id}.json")
...
def fetch_comments(session, subdomain, ticket_id, newest_first=True, per_page=100,
                   attempts=6, required=True):
    """The ticket's comments; None when they cannot be read and `required` is False.

    Newest first by default: the marker that stops a re-run from writing twice is on
    the most recent comment, and one page of a busy ticket would otherwise be all
    opening back-and-forth.

    `required` exits on any failure: a command acting on one ticket has nothing to do
    without them. A digest enriching many tickets passes False and skips the one it
    cannot read.
    """
    url = api_url(subdomain, f"tickets/{ticket_id}/comments.json")
    params = {"per_page": per_page, "sort_order": "desc" if newest_first else "asc"}
    if required:
        resp = request_with_retry(session, "GET", url, attempts=attempts, params=params)
        if resp.status_code >= 400:
...
def count_tickets(session, subdomain, query):
    """How many tickets match `query`. Best effort: None on failure.

    Context for a digest header, not something to hold the run up for, hence the
    short retry budget.
    """
    url = api_url(subdomain, "search/count.json")
    try:
        resp = request_with_retry(
            session, "GET", url, attempts=2, params={"query": query}
        )
        if resp.status_code >= 400:
            print(f"Note: could not count {query!r} ({resp.status_code}).")
            return None
        ...
        print(f"Note: could not count {query!r} ({exc}).")
        return None
...
def fetch_user(session, subdomain, user_id):
    ...
    """
    url = api_url(subdomain, f"users/{user_id}.json")
...
def hydrate_requester_activity(session, subdomain, tickets):
    ...
    for start in range(0, len(ids), 100):
        ...
        url = api_url(subdomain, "tickets/show_many.json")
        try:
...
def update_ticket(session, subdomain, ticket_id, fields, attempts=6):
    """PUT one ticket update; returns the response.

    Callers never print its body: Zendesk echoes a submitted comment back in a 422,
    and a comment can be a reply to a customer.
    """
    return request_with_retry(session, "PUT", api_url(subdomain, f"tickets/{ticket_id}.json"),
                              attempts=attempts, json={"ticket": fields})
...
def api_user_id(session, subdomain):
    ...
    """
    url = api_url(subdomain, f"users/me.json")
    resp = request_with_retry(session, "GET", url)
    if resp.status_code >= 400:
...
def change_tags(session, subdomain, ticket_id, add=(), drop=()):
    ...
    """
    url = api_url(subdomain, f"tickets/{ticket_id}/tags.json")
...
def conversation_turns(session, subdomain, ticket):
    ...
    if requester is None:
        ...
        return None
    comments = fetch_comments(session, subdomain, ticket["id"], newest_first=False,
                              attempts=2, required=False)
    if comments is None:
        return None
