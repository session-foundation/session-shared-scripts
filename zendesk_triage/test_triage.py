...
import claude_cli  # noqa: E402  (needs the path inserts above)
import transcript  # noqa: E402
import triage  # noqa: E402
import zendesk  # noqa: E402
from shared import discord  # noqa: E402
from shared.testing import (  # noqa: E402
...
def text_displays(node):
    ...
    if not isinstance(node, dict):
        return []
    found = [node["content"]] if node.get("type") == discord.TEXT_DISPLAY else []
    ...
    def test_store_reviews_are_never_in_scope(self):
        ...
        for query in (triage.build_window_query(72), triage.DEFAULT_QUERY):
            with self.subTest(query=query):
                self.assertIn(f"-via:{zendesk.REVIEW_CHANNEL}", query)
...
class TestRequesterActivity(unittest.TestCase):
    ...
    def test_it_attaches_the_requester_timestamp(self):
        ...
        self.assertEqual(zendesk.hydrate_requester_activity(session, "acme", tickets), 2)
    ...
    def test_it_sideloads_rather_than_fetching_each_ticket(self):
        ...
        zendesk.hydrate_requester_activity(session, "acme", [ticket(1)])
    ...
    def test_it_batches_at_a_hundred_ids(self):
        ...
        zendesk.hydrate_requester_activity(session, "acme", tickets)
    ...
    def test_a_failed_sideload_leaves_the_ticket_alone(self):
        ...
        for response in (FakeResponse({}, status_code=500), NonJsonResponse(),
            ...
            with self.subTest(response=type(response).__name__):
                with NoSleep():
                    zendesk.hydrate_requester_activity(
...
class TestReviewFiltering(unittest.TestCase):
    ...
    def test_star_count_is_read_from_the_subject(self):
        self.assertEqual(zendesk.review_stars(self.review(1, 5)), 5)
        self.assertEqual(zendesk.review_stars(self.review(2, 1)), 1)

    def test_non_review_subject_has_no_stars(self):
        self.assertIsNone(zendesk.review_stars(ticket(1, subject="Notifications broken")))
    ...
    def test_channel_identifies_a_review_without_stars_in_the_subject(self):
        ...
        self.assertTrue(zendesk.is_store_review(
            ticket(1, subject="no stars here", via={"channel": "any_channel"})))

    def test_web_tickets_are_not_reviews(self):
        self.assertFalse(zendesk.is_store_review(ticket(1, via={"channel": "web"})))
...
class TestReviewPlatform(unittest.TestCase):
    ...
    def test_google_play_is_android(self):
        self.assertEqual(zendesk.review_platform(self.google_play(1)), "android")
    ...
    def test_the_app_store_is_ios(self):
        ...
        self.assertEqual(zendesk.review_platform(self.app_store(1)), "ios")

    def test_a_review_naming_no_store_stays_unresolved(self):
        self.assertIsNone(zendesk.review_platform(self.review(1)))
        self.assertIsNone(zendesk.review_platform(
    ...
    def test_non_review_tickets_are_not_a_source_of_platform(self):
        ...
        self.assertIsNone(zendesk.review_platform(email))
        self.assertIsNone(zendesk.review_platform(ticket(2, via={"channel": "web"})))
    ...
    def test_every_resolvable_source_maps_to_a_known_platform(self):
        for _, platform in zendesk.REVIEW_SOURCE_PLATFORMS:
...
class TestContentFreeTickets(unittest.TestCase):
    ...
    def test_hydration_requests_a_bounded_page_of_comments(self):
        ...
        self.assertEqual(kwargs["params"], {"per_page": 10, "sort_order": "asc"})
...
class TestMessageCharLimit(unittest.TestCase):
    ...
    def test_a_full_message_stays_inside_the_character_ceiling(self):
        ...
        findings = [self.fat(i) for i in range(triage.MAX_HIGHLIGHTS)]
        for message in build_messages(findings, "acme"):
            self.assertLessEqual(all_text(message), discord.MAX_MESSAGE_TEXT_CHARS)
    ...
    def test_chunking_splits_on_whichever_limit_binds_first(self):
        ...
        self.assertEqual(len(discord.chunk_entries(entries, triage.MAX_ENTRIES_PER_MESSAGE)), 2)   # characters
        lean = [("x", {i}) for i in range(triage.MAX_ENTRIES_PER_MESSAGE + 1)]
        self.assertEqual(len(discord.chunk_entries(lean, triage.MAX_ENTRIES_PER_MESSAGE)), 2)      # card count

    def test_an_oversized_entry_still_gets_a_message(self):
        chunks = discord.chunk_entries([("x" * (triage.MAX_COMPONENT_CHARS + 50), {1})],
                                       triage.MAX_ENTRIES_PER_MESSAGE)
...
class TestFetchTickets(unittest.TestCase):
    ...
    def test_max_tickets_caps_the_batch_but_not_the_reported_total(self):
        ...
        tickets, total = zendesk.fetch_tickets(session, "acme", "q", 4)
    ...
    def test_pagination_stops_at_the_zendesk_result_limit(self):
        ...
        """
        limit = zendesk.SEARCH_RESULT_LIMIT
        pages = [
            FakeResponse({
                "count": 5000,
        ...
        tickets, total = zendesk.fetch_tickets(session, "acme", "q", 5000)
    ...
    def test_max_tickets_below_the_limit_still_wins(self):
        ...
        tickets, _ = zendesk.fetch_tickets(session, "acme", "q", 7)
    ...
    def test_an_unexpected_422_keeps_the_tickets_already_fetched(self):
        ...
        tickets, total = zendesk.fetch_tickets(session, "acme", "q", 900)
        self.assertEqual([t["id"] for t in tickets], [1])
...
class TestFetchEveryTicket(unittest.TestCase):
    ...
    def test_a_full_slice_is_followed_by_another(self):
        ...
        first = list(range(zendesk.SEARCH_RESULT_LIMIT))
        session = FakeSession([
            self.page(first, 1036, "2026-08-02T00:00:00Z"),
            self.page(range(9000, 9036), 36, "2025-08-02T00:00:00Z"),
        ])
        tickets, total = zendesk.fetch_every_ticket(session, "acme", "q", 5000)
        self.assertEqual(len(tickets), zendesk.SEARCH_RESULT_LIMIT + 36)
    ...
    def test_the_overlapping_second_is_not_counted_twice(self):
        ...
        tickets, _ = zendesk.fetch_every_ticket(session, "acme", "q", 5000)
        self.assertEqual(len(tickets), len({t["id"] for t in tickets}))
        self.assertEqual(len(tickets), zendesk.SEARCH_RESULT_LIMIT + 2)
    ...
    def test_a_slice_that_adds_nothing_new_ends_the_walk(self):
        ...
        tickets, _ = zendesk.fetch_every_ticket(session, "acme", "q", 99999)
        self.assertEqual(len(tickets), zendesk.SEARCH_RESULT_LIMIT)
        self.assertEqual(len(session.calls), 2)

    def test_it_never_returns_more_than_asked_for(self):
        session = FakeSession([self.page(range(10), 10)])
        tickets, _ = zendesk.fetch_every_ticket(session, "acme", "q", 4)
        self.assertEqual(len(tickets), 4)
...
class TestBacklogQueries(unittest.TestCase):
    def test_the_non_review_query_is_the_backlog_minus_the_review_channel(self):
        ...
        self.assertIn(f"-via:{zendesk.REVIEW_CHANNEL}", triage.BACKLOG_NON_REVIEW_QUERY)

    def test_the_counter_honours_the_query_it_is_given(self):
        session = FakeSession([FakeResponse({"count": 428})])
        count = zendesk.count_tickets(session, "acme", triage.BACKLOG_NON_REVIEW_QUERY)
...
class TestFetchTotalUnsolved(unittest.TestCase):
    def test_returns_the_count(self):
        ...
        self.assertEqual(zendesk.count_tickets(session, "acme", triage.BACKLOG_QUERY), 5609)
    ...
    def test_failure_is_non_fatal(self):
        ...
        self.assertIsNone(zendesk.count_tickets(session, "acme", triage.BACKLOG_QUERY))
    ...
    def test_uses_a_short_retry_budget(self):
        ...
        zendesk.count_tickets(session, "acme", triage.BACKLOG_QUERY)
        self.assertEqual(len(session.calls), 2)
...
class TestClaudeCli(unittest.TestCase):
    ...
    def run_cli(self, response=None, returncode=0, stderr="", raises=None,
        ...
        with Patched(claude_cli.subprocess, run=fake_run):
            return claude_cli.run_json("claude-opus-5", "medium", "be terse",
...
class TestEnglishTranscript(unittest.TestCase):
    ...
    def turns(self, comments, requester_id=5):
        ...
        with Patched(zendesk, request_with_retry=lambda *a, **k: Resp()):
            return zendesk.conversation_turns(object(), "acme",
    ...
    def test_an_unparseable_timestamp_leaves_the_turn_labelled(self):
        self.assertEqual(zendesk.stamp_minutes("not a date"), "")
        self.assertEqual(zendesk.stamp_minutes(None), "")
    ...
    def test_the_stamp_is_minutes_not_seconds(self):
        ...
        self.assertEqual(zendesk.stamp_minutes("2026-08-28T01:31:09Z"),
    ...
    def test_nothing_happens_until_the_field_exists(self):
        ...
        writes."""
        calls = []
        with Patched(zendesk, conversation_turns=lambda *a: calls.append(a)), \
                Patched(claude_cli, run_json=lambda *a, **k: calls.append(a)):
    ...
    def test_a_run_that_posts_no_card_writes_nothing(self):
        ...
        call = source.index("transcript.attach_english(")
    ...
    def test_a_run_that_wrote_nothing_says_so(self):
        ...
        with Patched(zendesk, conversation_turns=lambda *a: None):
    ...
    def test_one_ticket_that_cannot_be_rendered_does_not_stop_the_digest(self):
        ...
        written = []
        with Patched(zendesk, conversation_turns=lambda *a: [
                        {"index": 0, "who": "Customer", "when": "", "body": "x"}]), \
                Patched(claude_cli, run_json=explode), \
                Patched(transcript, write_english_field=lambda *a: written.append(a) or True):
    ...
    def test_the_transcript_is_written_to_the_configured_field(self):
        ...
        with Patched(zendesk, conversation_turns=lambda *a: [
                        {"index": 0, "who": "Customer", "when": "2026-08-28 00:22 UTC",
                         "body": "Es geht nicht."}]), \
                Patched(claude_cli, run_json=lambda *a, **k: {
                    "turns": [{"index": 0, "english": "It does not work."}]}), \
                Patched(transcript, write_english_field=lambda *a: puts.append(a) or True):
            got = transcript.attach_english(
                object(), "acme", [{"id": 1}, {"id": 2}],
    ...
    def test_a_ticket_with_no_matching_row_is_skipped(self):
        ...
        with Patched(zendesk, conversation_turns=lambda *a: [
                        {"index": 0, "who": "Customer", "when": "", "body": "x"}]), \
                Patched(claude_cli, run_json=lambda *a, **k: {"turns": []}), \
                Patched(transcript, write_english_field=lambda *a: True):
            self.assertEqual(
...
class TestCustomerSide(unittest.TestCase):
    ...
    def test_an_ordinary_ticket_costs_no_lookups(self):
        session = FakeSession([])
        authors = zendesk.customer_authors(
    ...
    def test_a_dm_falls_back_to_whoever_is_not_an_agent(self):
        ...
        authors = zendesk.customer_authors(session, "acme", self.TWEET, [
    ...
    def test_an_unresolvable_author_counts_as_the_customer(self):
        session = FakeSession([FakeResponse({}, status_code=404)])
        self.assertEqual(
            zendesk.customer_authors(session, "acme", self.TWEET,
                                    [self.said("中国大陆", -1)]), {-1})

    def test_the_sample_keeps_their_words_and_drops_the_agent_s(self):
        session = FakeSession([FakeResponse({"user": {}}),
                               FakeResponse({"user": {"id": 7, "role": "admin"}})])
        sample = zendesk.customer_text(session, "acme", self.TWEET, [
            self.said("中国大陆可以使用吗？", -1),
    ...
    def test_private_notes_never_reach_the_sample(self):
        session = FakeSession([])
        sample = zendesk.customer_text(session, "acme", self.EMAIL,
    ...
    def test_a_ticket_with_no_text_still_yields_a_sample(self):
        session = FakeSession([])
        self.assertTrue(zendesk.customer_text(
