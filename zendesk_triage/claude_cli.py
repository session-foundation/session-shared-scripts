"""One schema-enforced request to the Claude Code CLI.

Authentication is whatever `claude` is already logged in as, so no caller holds a
key. run_json exits on any failure, which is right for a classification the run
cannot continue without; try_run_json is for an enrichment, and hands the failure
back instead.
"""
import json
import os
...
# Shorthands for the model override, so ZENDESK_TRIAGE_MODEL=sonnet works for a big
...
def run_json(model, effort, system_prompt, schema, prompt, timeout, label):
...
def try_run_json(*args, **kwargs):
    """run_json for an enrichment: (payload, None), or (None, why) when the call failed."""
    try:
        return run_json(*args, **kwargs), None
