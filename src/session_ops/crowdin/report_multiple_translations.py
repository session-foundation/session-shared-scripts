...
Usage:
    crowdin-report-duplicates --dry-run        # scan + print, post nothing
    crowdin-report-duplicates                  # scan + post to Discord
    crowdin-report-duplicates --json multi.json # also write findings JSON
"""
import argparse
import collections
...
def crowdin_client(token, project_id):
    """Ten attempts at sixty seconds: a locale is ~1,400 requests near Crowdin's rate
    limit, so a 429 partway through is expected rather than exceptional. A 4xx
    raises, so a caller can read the body of what it asked for without checking."""
...
def post_to_discord(webhook_url, messages):
    ...
    # sent to Discord.
    with http.Session() as webhook_session:
