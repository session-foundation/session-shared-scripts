...
Usage:
    ...
    crowdin-approve-strings --list ongoingAppeal ongoingAppealDescription

    # approve ONLY translations submitted by trusted users (by username or id):
    crowdin-approve-strings --by-user alice --by-user 12345 ongoingAppeal ongoingAppealDescription

    # dry-run of the above (shows what would be approved, changes nothing):
    crowdin-approve-strings --dry-run --by-user alice ongoingAppeal ongoingAppealDescription
...
def crowdin_client(token, project_id):
    return sdk.client(token, project_id, timeout=30)
    ...
    for ident in identifiers:
        ...
        while True:
            data = client.source_strings.list_strings(
                filter=ident, scope="identifier", limit=500, offset=offset)["data"]
            for row in data:
    ...
    for ident, sid in string_ids.items():
        for lang in target_langs:
            ...
            try:
                ...
                body = sdk.response_text(exc)
                if exc.http_status == 400 and "already" in body.lower():
                    print(f"  [ -- ] {ident} / {lang}: already approved {info}")
                    already += 1
                else:
                    print(f"  [ERR ] {ident} / {lang}: {exc.http_status} {body}")
                    errors += 1
                continue
