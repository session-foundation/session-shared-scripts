...
def client(api_responses=(), download_responses=()):
    ...
    crowdin.session = FakeSession(api_responses)
...
class TestRequests(unittest.TestCase):
    def test_the_token_goes_to_the_api_and_never_to_the_export_host(self):
        crowdin = download.Crowdin("secret", "618696", max_workers=4)
        self.assertEqual(crowdin.session.headers["Authorization"], "Bearer secret")
    ...
    def test_a_server_error_is_retried(self):
        ...
        with NoSleep():
            self.assertEqual(crowdin.request("GET", "projects/618696", "project")["data"], {"id": 1})
        self.assertEqual(len(crowdin.session.calls), 2)
        self.assertEqual(crowdin.session.calls[0][1], f"{download.API}/projects/618696")
    ...
    def test_a_client_error_names_the_step_and_crowdins_message(self):
        ...
        with self.assertRaises(download.CrowdinError) as caught:
            crowdin.request("GET", "projects/618696", "Failed to retrieve project details")
    ...
    def test_a_non_json_error_page_still_reads_as_an_error(self):
        ...
        with self.assertRaises(download.CrowdinError) as caught:
            client([page]).request("GET", "projects/618696", "project")
...
class TestExport(unittest.TestCase):
    def test_a_language_is_exported_then_saved_under_its_locale(self):
        ...
        method, url, kwargs = crowdin.session.calls[0]
        self.assertEqual((method, url), ("POST", f"{download.API}/projects/618696/translations/exports"))
        self.assertEqual(kwargs["json"], {"targetLanguageId": "de", "format": "xliff",
    ...
    def test_the_source_language_is_always_exported_whole(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            download.export_and_download_language(
                ...
                skip_untranslated=True, allow_unapproved=False)
        payload = crowdin.session.calls[0][2]["json"]
