            ...
            if context_group is not None:
                ...
                if plural_form is not None:
                    ...
                    if target is not None and target.text:
                        ...
                        print(
                            f"Warning: Using source text for plural form '{form}' of '{resname}' in '{target_language}' as target is missing or empty"
                        )
...
def convert_placeholders_for_plurals(translations: Dict[str, str], glossary_dict: Dict[str, str]):
    ...
    for form, value in translations.items():
        converted_translations[form] = clean_string(
            value,
            False,
...
def convert_xliff_to_string_catalog(input_dir, output_dir, source_language, target_languages, glossary_dict):
    string_catalog = {"sourceLanguage": "en", "strings": {}, "version": "1.0"}
    ...
    for language in sorted_languages:
        ...
        print(
            f"\033[2K{Fore.WHITE}⏳ Converting translations for {target_language} to target format...{Style.RESET_ALL}",
            end="\r",
        )
